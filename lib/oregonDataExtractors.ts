...
const oregonResidentialPurchaseAgreementInfoExtractor: {
  [key: string]: (input: ExtractedAnswers) => string;
} = {
  earnest_money: (ans) => getEarnestMoneyText(ans.earnest_money as boolean),
  purchase_property_tax_lot: (ans) => `${ans.purchase_property_tax_lot}`,
  personal_property_included: (ans) => getPersonalPropertyText(ans),
  ...
  closing_costs_responsibility: (ans) =>
    getClosingCostResponsibility(ans.closing_costs_responsibility as string),
  ...
  possession_and_tenants: ans => getPosessionAndTenants(ans),
  has_loan: ans => getLoanText(ans.has_loan as boolean)
};

function getPersonalPropertyText(ans: ExtractedAnswers): string {
...
function getClosingCostResponsibility(userChoice: string): string {
...
function getPosessionAndTenants(ans: ExtractedAnswers): string {
...
function getLoanText(userChoice: boolean): string {
  if (userChoice) {
...
function getEarnestMoneyText(userChoice: boolean): string {
  if (userChoice) {
